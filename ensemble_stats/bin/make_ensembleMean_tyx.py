...
def do_regrid(variable, lstInFile, outdir, stringBefore, yearStart, yearEnd, resolution, topLevel=0, bottomLevel=1000):
    ...
    if lstInFile is None:
        ...
        return None
    if len(lstInFile)==0:
        thisLogger.info('Found no file to regrid in ' + str(lstInFile) + ' , consider revising search pattern. Return.')
        return None
    ...
    #    print'5 ' + str(startTime) + '-' + str(endTime) + '-' + str(thisFile[variable].getTime().asComponentTime()) + '-' + str(startYear) + '-' + str(endYear)
...
#                data = cdms2.createVariable(thisFile[variable].subRegion( time=(startTime[0], endTime[-1], 'cc'), level=(topLevel, bottomLevel,'cc') )) 
        ...
        if thisFile[variable].getLevel() is None:
	    #print 'TEST2 ' + '@' + str(nodata) + '@' + str(thisFile[variable].getOrder()) 
...
def do_stats(variable, validYearList, monthList, lstInFile, outdir, stringBefore, outnameBase, minVar=-1.e20, maxVar=1.e20, doSTD=False):
    ...
    if len(lstInFile)==0:
        thisLogger.info('Found no file to stat in' + str(lstInFile) + ', consider revising search pattern.')
        return
...
if __name__=="__main__":
	...
	#print 'TEST ' + thisModel + ' ' + rcp + ' ' + str(lstInFile) + ' ' + str(startYear) + ' ' + str(endYear) + ' ' + str(resolution) + ' ' + str(topLevel) + ' ' + str(bottomLevel)
        if regridFirst:
